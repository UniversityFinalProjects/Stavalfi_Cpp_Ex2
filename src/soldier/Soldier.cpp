...
Soldier::Soldier(const Point2d &location, const std::string &playerId,
                 short lifePoints, short walkingSpeed) :
        MapObject(location), playerId(playerId), weapon(nullptr),
        lifePoints(lifePoints), walkingSpeed(walkingSpeed),
        armors(2, nullptr) {}


Soldier::Soldier(const Point2d &location, const std::string &playerId, Weapon *weapon, short lifePoints,
                 short walkingSpeed)
        : MapObject(location), playerId(playerId), weapon(weapon),
          lifePoints(lifePoints), walkingSpeed(walkingSpeed),
          armors(2, nullptr) {
    assert(weapon != nullptr);
}

bool Soldier::isEnemy(const Soldier &soldier) {
